package org.rutebanken.irkalla.routes.singleton;
...
public class CustomRoutePolicy extends RoutePolicySupport  implements CamelContextAware {
    ...
    public CustomRoutePolicy() {
        this(HazelcastUtil.newInstance(), true);
    }

    public CustomRoutePolicy(HazelcastInstance instance) {
        this(instance, false);
    }

    public CustomRoutePolicy(HazelcastInstance instance, boolean managedInstance) {
    ...
    private synchronized void stopConsumer(Route route) {
        try {
            if (!suspendedRoutes.contains(route)) {
                ...
                boolean result = stopConsumer(route.getConsumer());
